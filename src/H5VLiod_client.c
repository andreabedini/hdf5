...
H5VL_iod_request_complete(H5VL_iod_file_t *file, H5VL_iod_request_t *req)
{
    ...
    case HG_DSET_OPEN:
        {
            ...
            if(IOD_OH_UNDEFINED == dset->remote_dset.iod_oh.rd_oh.cookie) {
                ...
                if(NULL == (plist = (H5P_genplist_t *)H5I_object(dset->remote_dset.dcpl_id)))
                    HGOTO_ERROR(H5E_ATOM, H5E_BADATOM, FAIL, "can't find object for ID");

                /* Get layout info */
                if(H5P_get(plist, H5D_CRT_LAYOUT_NAME, &layout) < 0)
                    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get property value for layout")
    ...
    case HG_DSET_MULTI_WRITE:
    case HG_DSET_MULTI_READ: /* Inclusion of read here is temporary */
        {
            H5VL_iod_multi_write_info_t *info = (H5VL_iod_multi_write_info_t *)req->data;
            size_t i;

            for(i = 0; i < info->count; i++) {
                /* Free memory handle */
                if(HG_SUCCESS != HG_Bulk_handle_free(*info->list[i].bulk_handle)) {
                    HERROR(H5E_FUNC, H5E_CANTINIT, "failed to free dataset bulk handle\n");
                    req->status = H5ES_STATUS_FAIL;
                    req->state = H5VL_IOD_COMPLETED;
                } /* end if */

                if(info->list[i].vl_len_bulk_handle && /* Remove this check from multi_write once multi_read has its own struct/case */
                   *info->list[i].vl_len_bulk_handle != HG_BULK_NULL && 
                   HG_SUCCESS != HG_Bulk_handle_free(*info->list[i].vl_len_bulk_handle)) {
                    fprintf(stderr, "failed to free dataset bulk handle\n");
                    req->status = H5ES_STATUS_FAIL;
                    req->state = H5VL_IOD_COMPLETED;
                } /* end if */
            ...
            for(i = 0; i < info->count; i++) {
                if(info->list[i].vl_lengths) {
                    ...
                    info->list[i].vl_lengths = NULL;
                } /* end if */
                info->list[i].bulk_handle = (hg_bulk_t *)H5MM_xfree(info->list[i].bulk_handle);
                info->list[i].vl_len_bulk_handle = (hg_bulk_t *)H5MM_xfree(info->list[i].vl_len_bulk_handle);
            } /* end for */

            info->list = (H5VL_iod_multi_write_info_ent_t *)H5MM_xfree(info->list);
            free(info->status);
            info->status = NULL;
            info = (H5VL_iod_multi_write_info_t *)H5MM_xfree(info);
            req->data = NULL;
            break;
        }
    case HG_DSET_READ:
    ...
    case HG_DSET_CLOSE:
        {
            ...
            if(dset->is_virtual)
                if(H5D__virtual_reset_storage(&dset->virtual_storage, FALSE) < 0)
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL, "unable to reset virtual layout")
    ...
    case HG_DSET_MULTI_READ: /* Inclusion of read here is temporary */
        {
            ...
            for(i = 0; i < info->count; i++) {
                ...
                /* Free memory handle */
                if(*info->list[i].vl_len_bulk_handle != HG_BULK_NULL &&
                        HG_SUCCESS != HG_Bulk_handle_free(*info->list[i].bulk_handle)) {
                    fprintf(stderr, "failed to free bulk handle\n");
                } /* end if */

                if(info->list[i].vl_lengths) {
                    HDfree(info->list[i].vl_lengths);
                    info->list[i].vl_lengths = NULL;
                } /* end if */
                info->list[i].bulk_handle = (hg_bulk_t *)H5MM_xfree(info->list[i].bulk_handle);
                info->list[i].vl_len_bulk_handle = (hg_bulk_t *)H5MM_xfree(info->list[i].vl_len_bulk_handle);
...

/*-------------------------------------------------------------------------
 * Function:    H5VL_iod_vds_pre_io
 *
 * Iterates over all VDS mappings, calculating the number of mappings
 * involved in I/O, the total number of elements, opening source datasets,
 * and creating projected_mem_space.
 *
 ...
 */
herr_t
H5VL_iod_vds_pre_io(H5VL_iod_dset_t *dset, const H5S_t *mem_space,
    ...
    /*out*/hsize_t *tot_nelmts)
{
    ...
    *virtual_count = 0;
    ...
    for(i = 0; i < dset->virtual_storage.list_nused; i++) {
        HDassert(dset->virtual_storage.list[i].source_dset.clipped_virtual_select);

        /* Project intersection of file space and mapping virtual space onto
         * memory space */
        if(H5S_select_project_intersection(file_space, mem_space, dset->virtual_storage.list[i].source_dset.clipped_virtual_select, &dset->virtual_storage.list[i].source_dset.projected_mem_space) < 0)
        ...
        if((select_nelmts = (hssize_t)H5S_GET_SELECT_NPOINTS(dset->virtual_storage.list[i].source_dset.projected_mem_space)) < 0)
        ...
        if(select_nelmts > (hssize_t)0) {
            ...
            if(!dset->virtual_storage.list[i].source_dset.dset) 
                /* Try to open source dataset synchronously */
                dset->virtual_storage.list[i].source_dset.dset = H5VL_iod_dataset_open(dset->common.file, loc_params, dset->virtual_storage.list[i].source_dset.dset_name, dset->dapl_id, dxpl_id, NULL);
        ...
        if(select_nelmts == (hssize_t)0) {
            if(H5S_close(dset->virtual_storage.list[i].source_dset.projected_mem_space) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL, "can't close projected memory space")
            dset->virtual_storage.list[i].source_dset.projected_mem_space = NULL;
        } /* end if */
        else {
            (*virtual_count)++;
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5VL_iod_vds_pre_io() */


/*-------------------------------------------------------------------------
 * Function:    H5VL_iod_vds_post_io
 *
 * Frees memory allocated by H5VL_iod_vds_pre_io()
 *
 * Return:      Success:        SUCCEED 
 *              Failure:        Negative
 *
 ...
 */
herr_t
H5VL_iod_vds_post_io(H5VL_iod_dset_t *dset, size_t virtual_count)
{
    size_t i, j;
    herr_t ret_value = SUCCEED;
    ...
    for(i = 0, j = 0;
        ...
        if(dset->virtual_storage.list[i].source_dset.projected_mem_space) {
            if(H5S_close(dset->virtual_storage.list[i].source_dset.projected_mem_space) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL, "can't free projected memory space")
            dset->virtual_storage.list[i].source_dset.projected_mem_space = NULL;
